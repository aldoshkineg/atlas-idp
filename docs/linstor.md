...
helm upgrade --install piraeus-operator \
  ...
  --values gitops/platform-kind/layers/storage/linstor/operator-values.yaml \
  --wait
...
helm upgrade --install linstor-cluster \
  ...
  --version 1.1.1 \
  --namespace piraeus-datastore \
  --values gitops/platform-kind/layers/storage/linstor/linstor-cluster-values.yaml \
  --wait
```
...
helm template piraeus-operator oci://ghcr.io/piraeusdatastore/piraeus-operator/piraeus \
  ...
  --values gitops/platform-kind/layers/storage/linstor/operator-values.yaml > /tmp/piraeus-rendered.yaml
...
| File                                                                      | Chart                         |
| ------------------------------------------------------------------------- | ----------------------------- |
| `gitops/platform-kind/layers/storage/linstor/operator-values.yaml`        | `piraeus-operator` (OCI)      |
| `gitops/platform-kind/layers/storage/linstor/linstor-cluster-values.yaml` | `linstor-cluster` (Helm repo) |
